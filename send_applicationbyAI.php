...
if (isset($_POST['Suitability']) && !empty($_POST['Suitability'])) {
    $suitability = $_POST['Suitability'];
} else {
    $suitability = 0; // หรือกำหนดค่าเริ่มต้นในกรณีที่ไม่มีค่า
}
...
if ($_SERVER["REQUEST_METHOD"] == "POST" ) {
    ...
    try {
        ...
        $insertQuery = "INSERT INTO job_applications (account_id, job_id, application_date, Suitability) VALUES (:account_id, :job_id, NOW(), :suitability)";
        ...
        if ($insertStmt->rowCount() > 0) {
            ...
            // ตรวจสอบสถานะการส่ง
            if ($mail->send()) {
                echo "<script>alert('ส่งอีเมลเรียบร้อยแล้ว'); window.location.href='af_job_searchbyAI.php';</script>";
                exit();
            } else {
                echo "<script>alert('ไม่สามารถส่งอีเมลได้: {$mail->ErrorInfo}'); window.location.href='af_job_searchbyAI.php';</script>";
                exit();
            }
        } else {
